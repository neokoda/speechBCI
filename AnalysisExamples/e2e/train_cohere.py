#!/usr/bin/env python3
"""Cohere Transcribe cross-attention BCI training (Stage 3).

Mirrors train_whisper.py exactly — only the model + tokenizer differ. Phase 1 warms
up the projector with the Cohere decoder frozen; phase 2 jointly fine-tunes the
ECoG encoder + projector + decoder-attention LoRA.

Phase 1 (projector warmup):
    python AnalysisExamples/e2e/train_cohere.py \\
        --data-dir data/derived/tfRecords \\
        --cohere-repo CohereLabs/cohere-transcribe-03-2026 \\
        --output-dir experiments/e2e_cohere \\
...
# ---------------------------------------------------------------------------
# Utilities (1:1 copy of train_whisper.py helpers, parameterized for Cohere)
# ---------------------------------------------------------------------------
...
def _trainable_state_dict(model):
    """Encoder + projector + LoRA only — skip frozen Cohere base weights."""
...
def train(args):
    ...
    print(f"Loading tokenizer: {args.cohere_repo}")
    ...
    freeze_cohere  = (args.phase == 1)
    freeze_encoder = (args.phase == 1) or getattr(args, "freeze_encoder", False)
    print(f"Phase {args.phase}: freeze_cohere={freeze_cohere}, freeze_encoder={freeze_encoder}")
    ...
    lora_params = [p for n, p in model.cohere.named_parameters()
...
def parse_args():
    p = argparse.ArgumentParser(description="Cohere Transcribe cross-attention BCI training")
    ...
    p.add_argument("--lr-encoder",        type=float, default=2e-4)
    p.add_argument("--lr-projector",      type=float, default=1.5e-3)
