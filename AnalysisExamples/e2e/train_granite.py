#!/usr/bin/env python3
"""Training script for GraniteBCIModel (IBM Granite Speech 4.1-2B).

Phase 1 (projector warmup):
    python AnalysisExamples/e2e/train_granite.py \\
        --data-dir data/derived/tfRecords \\
        --output-dir experiments/e2e_granite \\
...
Phase 2 (joint fine-tuning):
    python AnalysisExamples/e2e/train_granite.py \\
        --data-dir data/derived/tfRecords \\
        --output-dir experiments/e2e_granite \\
        --phase 2 --max-steps 15000 --batch-size 8
"""
...
def train(args):
    ...
    # ── Tokenizer ─────────────────────────────────────────────────────────
    print(f"Loading tokenizer: {args.granite_model}")
    ...
    freeze_encoder = (args.phase == 1) or args.freeze_encoder
...
def parse_args():
    p = argparse.ArgumentParser(description="GraniteBCI training")
    p.add_argument("--data-dir",    required=True)
    p.add_argument("--output-dir",  required=True)
    p.add_argument("--granite-model", default="ibm-granite/granite-speech-4.1-2b",
                   help="HuggingFace Granite Speech model id")
