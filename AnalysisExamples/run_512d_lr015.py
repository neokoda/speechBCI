#!/usr/bin/env python3
"""
Full training run: 512d_4L_8H_2048ff with LR=0.015 cosine schedule.

Tests whether the larger 512d model benefits from the same high LR that
improved the 256d model (0.367 -> 0.316). The 512d model was previously
only tested at LR=0.0005 (PER=0.383).

Usage:
    python run_512d_lr015.py \
...
CONFIG = {
    'name': 'transformer_512d_4L_8H_2048ff_lr015',
    'd_model': 512, 'num_layers': 4, 'nhead': 8, 'd_ff': 2048,
}

FIXED = {
    'nBatchesToTrain':     200000,
    'batchesPerVal':       500,
    'batchSize':           32,
    'learnRateStart':      0.015,
    'learnRateEnd':        0.0015,
    'learnRateDecaySteps': 200000,
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  512d MODEL TRAINING: LR=0.015")
    ...
    print(f"  256d LR=0.015 baseline PER: {BASELINE_256D_LR015_PER}")
    ...
    while True:
        ...
        try:
            ...
            print(f"  Compare: 256d LR=0.015 = {BASELINE_256D_LR015_PER} PER")
            ...
            result = {
                ...
                'baseline_256d_lr015_per': BASELINE_256D_LR015_PER,
                'gru_baseline_per': GRU_BASELINE_PER,
            }
            with open(os.path.join(args.output_dir, '512d_lr015_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='512d model training: LR=0.015')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/512d_lr015')
