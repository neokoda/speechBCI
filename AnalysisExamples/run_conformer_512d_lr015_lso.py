#!/usr/bin/env python3
"""
Conformer training: 512d_4L_8H_2048ff with LR=0.015 cosine + LossScaleOptimizer.

Tests whether Conformer architecture (conv + attention) improves over pure
Transformer (PER=0.2754). Same hyperparameters as best Transformer run.

Usage:
    python run_conformer_512d_lr015_lso.py \
        --data-dir /workspace/speechBCI/data/derived/tfRecords \
        --output-dir /workspace/speechBCI/experiments/conformer_512d_lr015_lso \
        --gpu 0
"""
...
CONFIG = {
    'name': 'conformer_512d_4L_8H_2048ff_lr015_lso',
...
BASELINE_TRANSFORMER_PER = 0.2754
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  CONFORMER 512d TRAINING: LR=0.015 + LossScaleOptimizer")
    ...
    print(f"  Transformer baseline PER: {BASELINE_TRANSFORMER_PER}")
    ...
    while True:
        ...
        try:
            ...
            print(f"  Compare: Transformer baseline = {BASELINE_TRANSFORMER_PER} PER")
            ...
            result = {
                ...
                'baseline_transformer_per': BASELINE_TRANSFORMER_PER,
                'gru_baseline_per': GRU_BASELINE_PER,
            }
            with open(os.path.join(args.output_dir, 'conformer_512d_lr015_lso_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Conformer 512d training: LR=0.015 + LossScaleOptimizer')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/conformer_512d_lr015_lso')
