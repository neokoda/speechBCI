...
def build_command(data_dir, output_dir, gpu):
    ...
    cmd = [
        ...
        f'adamEpsilon={CONFIG["adam_epsilon"]}',
        'mixedPrecision=true',
        f'outputDir={exp_dir}',
        f'gpuNumber="{gpu}"',
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  CONFORMER + ADAM EPSILON=1e-7")
    ...
    print(f"  Adam epsilon: {CONFIG['adam_epsilon']} (default was 0.1)")
    ...
    while True:
        ...
        try:
            ...
            with open(os.path.join(args.output_dir, 'conformer_eps_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Conformer + Adam epsilon=1e-7')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/conformer_eps')
