...
def build_command(data_dir, output_dir, gpu):
    ...
    cmd = [
        ...
        'model=conformer_stack_inputNet',
        ...
        f'model.convKernelSize={CONFIG["conv_kernel_size"]}',
        'model.dropout=0.1',
        'model.posEncType=sinusoidal',
        'model.gradientCheckpointing=false',
        'model.specAugment=true',
        'model.freqMaskParam=27',
        'model.timeMaskParam=10',
        'model.nFreqMasks=2',
        'model.nTimeMasks=2',
        'model.squeezeExcitation=true',
        'model.seNChannels=256',
        f'model.seReduction={CONFIG["se_reduction"]}',
        f'model.stack_kwargs.kernel_size={CONFIG["stack_kernel_size"]}',
        f'model.stack_kwargs.strides={CONFIG["stack_strides"]}',
        'mixedPrecision=true',
        f'outputDir={exp_dir}',
        f'gpuNumber="{gpu}"',
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  CONFORMER + STACK KERNEL_SIZE=14 (matching GRU)")
    print(f"  Config: {CONFIG['name']}")
    print(f"  d_model={CONFIG['d_model']}, layers={CONFIG['num_layers']}, "
          f"heads={CONFIG['nhead']}, d_ff={CONFIG['d_ff']}, "
          f"conv_kernel={CONFIG['conv_kernel_size']}")
    print(f"  Stack: kernel_size={CONFIG['stack_kernel_size']}, "
          f"strides={CONFIG['stack_strides']} "
          f"(input dim: {CONFIG['stack_kernel_size']*256})")
    print(f"  SE: reduction={CONFIG['se_reduction']}")
    print(f"  LR: {FIXED['learnRateStart']} -> {FIXED['learnRateEnd']} "
          f"(cosine over {FIXED['learnRateDecaySteps']} steps)")
    print(f"  Dropout: 0.1")
    print(f"  SpecAugment: freq_mask=27, time_mask=10, n_freq=2, n_time=2")
    print(f"  Previous best PER: {PREV_BEST_PER}")
    ...
    while True:
        ...
        try:
            ...
            print(f"  Compare: Previous best = {PREV_BEST_PER} PER")
            ...
            result = {
                ...
                'spec_augment': {
                    'freq_mask_param': 27, 'time_mask_param': 10,
                ...
                'stack_kwargs': {
                    ...
                    'strides': CONFIG['stack_strides'],
                    'input_dim': CONFIG['stack_kernel_size'] * 256,
                },
                'best_per': per,
                'best_step': best_step,
                ...
                'prev_best_per': PREV_BEST_PER,
                'gru_baseline_per': GRU_BASELINE_PER,
            }
            with open(os.path.join(args.output_dir, 'conformer_k14_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Conformer + stack kernel_size=14')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/conformer_k14')
