...
CONFIG = {
    'name': 'conformer_512d_4L_8H_2048ff_shortLR',
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  CONFORMER SHORT LR CYCLE (100k) + NO GRAD CHECKPOINTING")
    ...
    print(f"  Max steps: {FIXED['nBatchesToTrain']}, warmup: {FIXED['warmUpSteps']}")
    print(f"  Patience: {FIXED['earlyStopPatience']}")
    print(f"  Gradient Checkpointing: OFF (15% faster)")
    ...
    while True:
        ...
        try:
            ...
            with open(os.path.join(args.output_dir, 'conformer_short_lr_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Conformer: shorter LR cycle + no grad checkpointing')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/conformer_short_lr')
