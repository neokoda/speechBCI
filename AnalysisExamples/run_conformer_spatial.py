...
CONFIG = {
    'name': 'conformer_512d_4L_spatial',
...
def build_command(data_dir, output_dir, gpu):
    ...
    cmd = [
        ...
        'model.nFreqMasks=2',
        'model.nTimeMasks=2',
        'model.squeezeExcitation=false',
        'model.spatialAttention=true',
        f'model.spatialAttnDim={CONFIG["spatial_attn_dim"]}',
        f'model.spatialAttnHeads={CONFIG["spatial_attn_heads"]}',
        'mixedPrecision=true',
        f'outputDir={exp_dir}',
        f'gpuNumber="{gpu}"',
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  CONFORMER + SPATIAL ATTENTION (cross-channel)")
    ...
    print(f"  Spatial Attention: d_attn={CONFIG['spatial_attn_dim']}, "
          f"heads={CONFIG['spatial_attn_heads']}")
    print(f"  SE: OFF (spatial attention replaces it)")
    ...
    while True:
        ...
        try:
            ...
            result = {
                ...
                'spatial_attention': {
                    'd_attn': CONFIG['spatial_attn_dim'],
                    'heads': CONFIG['spatial_attn_heads'],
                },
                'best_per': per,
                'best_step': best_step,
            ...
            with open(os.path.join(args.output_dir, 'conformer_spatial_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Conformer + Spatial Attention')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/conformer_spatial')
