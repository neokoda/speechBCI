...
CONFIG = {
    'name': 'conformer_256d_2L_win64',
    'd_model': 256, 'num_layers': 2, 'nhead': 4, 'd_ff': 512,
    'conv_kernel_size': 31,
    'attn_window': 64,
}
...
def build_command(data_dir, output_dir, gpu):
    ...
    cmd = [
        ...
        'model.spatialAttention=false',
        f'model.attnWindow={CONFIG["attn_window"]}',
        'mixedPrecision=true',
        f'outputDir={exp_dir}',
        f'gpuNumber="{gpu}"',
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  TINY CONFORMER + WINDOWED ATTENTION")
    ...
    print(f"  Attention window: +/- {CONFIG['attn_window']} positions")
    ...
    print(f"  SE / Spatial Attention: OFF")
    ...
    while True:
        ...
        try:
            ...
            with open(os.path.join(args.output_dir, 'conformer_tiny_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Tiny Conformer + windowed attention')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/conformer_tiny')
