...
FINAL_CONFIGS = [
    {
        ...
        'd_model': 256, 'num_layers': 4, 'nhead': 8, 'd_ff': 512,
    },
    {
        'name': 'transformer_512d_4L_8H_2048ff',
...
def run_experiments(args):
    ...
    print(f"\n{'='*70}")
    print(f"  FINAL TRAINING: Top 2 Transformer Configs")
    print(f"  {len(configs)} configs x {FIXED['nBatchesToTrain']} batches each")
    ...
    print(f"{'='*70}")
    print(f"{'Rank':>4} {'Config':<40} {'Val PER':>10} {'Step':>8} {'Status':>14}")
    print('-' * 80)
    ...
    with open(results_csv, 'w', newline='') as f:
        ...
        for rank, r in enumerate(results, 1):
            ...
            step_str = str(r.get('final_step', '?'))
            print(f"{rank:>4} {r['name']:<40} {per_str:>10} {step_str:>8} {r.get('status','?'):>14}")
            writer.writerow([
                rank, r['name'], r['d_model'], r['num_layers'], r['nhead'], r['d_ff'],
                r['val_per'], r.get('status', '?'),
    ...
    with open(final_results_file, 'w') as f:
        json.dump({
            'best_config': results[0]['name'] if results and results[0]['val_per'] < float('inf') else None,
            'all_results': results
        }, f, indent=2, default=str)

    if results and results[0]['val_per'] < float('inf'):
        print(f"\n  Best model: {results[0]['name']} (PER: {results[0]['val_per']:.4f})")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Final Training: Top 2 Transformer Configs (100k batches)')
    parser.add_argument('--data-dir', required=True,
                        help='Path to tfRecords directory')
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/final_training',
                        help='Output directory for final training experiments')
