...
CONFIG = {
    'name': 'transformer_256d_4L_8H_512ff_lr015',
...
def run(args):
    print(f"\n{'='*70}")
    print(f"  FULL TRAINING RUN: LR=0.015 (best probe)")
    ...
    print(f"  Cosine baseline PER: {COSINE_BASELINE_PER}")
    print(f"{'='*70}\n")
    ...
    while True:
        ...
        try:
            ...
            print(f"  Compare: cosine baseline = {COSINE_BASELINE_PER} PER")
            ...
            result = {
                ...
                'cosine_baseline_per': COSINE_BASELINE_PER,
                'gru_baseline_per': 0.169,
            }
            with open(os.path.join(args.output_dir, 'full_lr015_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Full training run: LR=0.015')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/full_lr015')
