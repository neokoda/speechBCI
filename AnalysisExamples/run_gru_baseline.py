...
def run(args):
    print(f"\n{'='*70}")
    print(f"  GRU BASELINE — TRAINING FROM SCRATCH")
    print(f"  Config: {CONFIG['name']}")
    print(f"  Units={CONFIG['nUnits']}, layers={CONFIG['nLayers']}, "
          f"dropout={CONFIG['dropout']}")
    print(f"  Stack: kernel_size={CONFIG['kernel_size']}, "
          f"strides={CONFIG['strides']}")
    print(f"  LR: {FIXED['learnRateStart']} -> {FIXED['learnRateEnd']} "
          f"(linear over {FIXED['learnRateDecaySteps']} steps)")
    print(f"  Batch size: {FIXED['batchSize']}")
    print(f"  Total batches: {FIXED['nBatchesToTrain']}")
    print(f"  Pre-trained checkpoint PER: {PRETRAINED_PER}")
    print(f"{'='*70}\n")
    ...
    try:
        ...
        print(f"  Compare: Pre-trained checkpoint PER = {PRETRAINED_PER}")
        print(f"  Compare: Best Conformer PER = 0.1839")
        print(f"{'='*70}")
        ...
        result = {
            ...
            'pretrained_per': PRETRAINED_PER,
        }
        with open(os.path.join(args.output_dir, 'gru_baseline_result.json'), 'w') as f:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='GRU baseline from scratch')
    parser.add_argument('--data-dir', required=True)
    parser.add_argument('--output-dir', default='/workspace/speechBCI/experiments/gru_baseline')
