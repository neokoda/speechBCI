...
Matriks fitur tersebut kemudian dinormalisasi dengan *z-score* per sesi. Normalisasi *z-score* dilakukan dengan mengurangkan nilai rata-rata lalu membaginya dengan simpangan baku yang dihitung untuk tiap kanal pada tiap sesi perekaman. Hasilnya, tiap kanal pada satu sesi memiliki rata-rata nol dan ragam satu sehingga perbedaan skala sinyal antarsesi yang dapat bernilai cukup besar menjadi berkurang (Willett et al., 2023).

Pada tahap pelatihan, diterapkan pula beberapa teknik tambahan pada matriks fitur. Teknik pertama adalah *Gaussian smoothing* pada dimensi waktu, yaitu konvolusi tiap kanal dengan *kernel* berbentuk kurva Gauss sehingga nilai tiap *bin* menjadi rata-rata berbobot dari *bin* di sekitarnya. Teknik ini memperhalus sinyal dan meredam fluktuasi berfrekuensi tinggi. Teknik kedua adalah penambahan *white noise*, yaitu penambahan derau acak dari distribusi normal ke nilai fitur agar model tidak terlalu bergantung pada nilai fitur yang persis. Teknik ketiga adalah penambahan *constant offset*, yaitu penambahan suatu nilai konstan yang sama ke seluruh *bin* pada satu kanal untuk meniru pergeseran nilai dasar sinyal. Teknik keempat adalah *subsampling* pada dimensi waktu, yaitu pemampatan beberapa *bin* berdekatan menjadi lebih sedikit langkah waktu, misalnya dengan faktor empat, agar panjang sekuens menjadi lebih ringkas sebelum diproses model.
...
Penjelasan tiap model dipaparkan sebagai berikut.

**1. Whisper.** Whisper adalah model *encoder-decoder* berbasis Transformer untuk pengenalan ucapan otomatis yang dikembangkan oleh OpenAI (Radford et al., 2022). Whisper dilatih pada sekitar 680 ribu jam audio multibahasa dan multitugas yang dikumpulkan dari web dengan supervisi lemah, yaitu pasangan audio dan transkrip yang tidak dikurasi secara ketat. Masukan audio terlebih dahulu diubah menjadi spektrogram log-mel. Spektrogram tersebut melewati dua lapisan konvolusi satu dimensi dengan fungsi aktivasi GELU, lalu ditambah *positional encoding* sinusoidal sebelum diproses oleh tumpukan blok *encoder* Transformer. *Decoder* Whisper membangkitkan token teks secara autoregresif. Pada tiap lapisan, *decoder* melakukan *self-attention* kausal pada token teks lalu mengakses keluaran *encoder* melalui *cross-attention*. Whisper memakai format multitugas dengan token khusus untuk menandai jenis tugas (transkripsi atau translasi), bahasa, dan ada tidaknya penanda waktu. Whisper tersedia dalam beberapa ukuran, dua di antaranya adalah Whisper-medium.en yang merupakan varian khusus bahasa Inggris berukuran sekitar 769 juta parameter dan Whisper-large-v3 berukuran sekitar 1,55 miliar parameter. Arsitektur Whisper ditunjukkan pada Gambar II.5.

![Arsitektur Whisper](image/TODO_THESIS/whisper.png)

**Gambar II.5** Arsitektur Whisper yang terdiri atas *encoder* audio dan *decoder* teks yang terhubung melalui *cross-attention* (Radford et al., 2022).

**2. Cohere Transcribe.** Cohere Transcribe adalah model pengenalan ucapan sumber terbuka dari Cohere yang dirilis pada tahun 2026 (Cohere, 2026). Model ini berarsitektur *encoder-decoder* yang memadukan *encoder* Conformer berukuran besar dengan *decoder* Transformer yang ringan. Audio terlebih dahulu diubah menjadi spektrogram log-mel. *Encoder* Conformer kemudian mengekstraksi representasi akustik dengan memadukan konvolusi untuk menangkap pola lokal dan *self-attention* untuk menangkap konteks global. Representasi tersebut diproses oleh *decoder* Transformer untuk membangkitkan token teks secara autoregresif. Untuk audio yang panjang, Cohere Transcribe memecah masukan menjadi potongan selebar sekitar 35 detik yang saling bertumpang tindih, lalu menyusun kembali hasilnya. Model ini memiliki sekitar 2 miliar parameter dan mendukung 14 bahasa. Arsitektur Cohere Transcribe ditunjukkan pada Gambar II.6.

![Arsitektur Cohere Transcribe](image/TODO_THESIS/cohere.png)

**Gambar II.6** Arsitektur Cohere Transcribe yang memadukan *encoder* Conformer besar dengan *decoder* Transformer ringan (Cohere, 2026).

**3. Qwen3.5-0.8B-Base.** Qwen adalah keluarga model bahasa besar *decoder-only* dari Alibaba yang dilatih pada teks berskala triliunan token dengan dukungan banyak bahasa (Qwen Team, 2025). Sebagai model *decoder-only*, Qwen memprediksi token berikutnya dari token-token sebelumnya secara autoregresif. Arsitekturnya mengikuti rancangan Transformer *decoder-only* dengan beberapa komponen khas, yaitu *grouped-query attention* untuk menekan kebutuhan memori, fungsi aktivasi SwiGLU pada lapisan *feed-forward*, normalisasi RMSNorm dengan pranormalisasi, serta *rotary positional embedding* (RoPE) untuk menyandikan posisi token. Salah satu variannya adalah Qwen3.5-0.8B-Base berukuran sekitar 0,8 miliar parameter. Imbuhan Base menandakan model hasil pralatih murni yang belum melewati penyetelan instruksi maupun penyelarasan preferensi sehingga keluarannya mengikuti pola data pralatihnya.

**4. Granite-Speech.** Granite-Speech adalah model bahasa besar audio-teks sumber terbuka dari IBM dengan kemampuan pengenalan ucapan bahasa Inggris yang kuat (Saon et al., 2025). Arsitekturnya terdiri atas tiga komponen utama. Komponen pertama adalah *encoder* akustik Conformer yang dilatih dengan CTC untuk mengekstraksi representasi dari audio. Komponen kedua adalah *adapter* modalitas berupa *query transformer* berjendela yang memampatkan representasi akustik pada dimensi waktu lalu memetakannya ke ruang *embedding* model bahasa. Komponen ketiga adalah model bahasa teks Granite yang diadaptasi dengan LoRA agar dapat memproses representasi ucapan. Granite-Speech tersedia dalam ukuran sekitar 2 miliar dan 8 miliar parameter yang masing-masing diturunkan dari model bahasa Granite berukuran sepadan.

**5. Canary-Qwen.** Canary-Qwen adalah model bahasa audio-teks dari NVIDIA yang dikategorikan sebagai *speech-augmented language model*, yaitu model bahasa yang diperluas agar dapat menerima masukan ucapan (NVIDIA, 2025). Model ini menggabungkan *encoder* FastConformer untuk memproses audio dengan model bahasa Qwen3-1.7B melalui proyeksi linier, sedangkan model bahasanya diadaptasi dengan LoRA. Canary-Qwen dilatih pada ratusan ribu jam audio bahasa Inggris dan dapat bekerja dalam dua mode, yaitu mode transkripsi ucapan dan mode model bahasa untuk tugas berbasis teks. Varian Canary-Qwen-2.5B berukuran sekitar 2,5 miliar parameter.

**6. LLaMA-2.** LLaMA-2 adalah keluarga model bahasa *decoder-only* dari Meta yang banyak digunakan sebagai dasar berbagai tugas pemrosesan bahasa (Touvron et al., 2023). Model ini dilatih pada sekitar dua triliun token teks dan tersedia dalam ukuran 7 miliar, 13 miliar, dan 70 miliar parameter. Arsitekturnya berupa Transformer *decoder-only* yang menggunakan normalisasi RMSNorm dengan pranormalisasi, fungsi aktivasi SwiGLU, dan *rotary positional embedding* (RoPE).
