...
4. **Conformer dengan *spatial attention*.** Model memiliki konfigurasi yang sama dengan Conformer vanila, tetapi ditambah modul *spatial attention* yang bekerja pada dimensi elektroda dengan dimensi representasi internal 64 dan 4 *head*. Mekanisme modul ini dijelaskan pada Bab III bagian III.3.2.

Nilai hiperparameter dekoder berbasis Transformer di atas, yaitu Transformer murni, Conformer vanila, dan Conformer dengan *spatial attention*, merupakan hasil penyetelan (*hyperparameter tuning*) dengan *grid search*. Hiperparameter yang disetel adalah dimensi *hidden*, jumlah lapisan, jumlah *attention head*, dimensi lapisan *feed-forward*, dan *dropout*. Untuk tiap hiperparameter dicoba beberapa nilai, misalnya dimensi *hidden* 256 dan 512, jumlah lapisan 4 dan 6, jumlah *head* 4 dan 8, serta dimensi *feed-forward* yang disesuaikan dengan dimensi *hidden*, lalu seluruh kombinasinya diuji.

Agar biaya komputasi terkendali, penyetelan memakai strategi *successive halving*. Mula-mula seluruh kombinasi kandidat dilatih dengan jumlah langkah yang kecil. Separuh kombinasi dengan PER validasi terburuk kemudian disingkirkan, sedangkan separuh sisanya dilatih ulang dengan jumlah langkah yang lebih besar. Proses menyingkirkan separuh kombinasi lalu melatih ulang sisanya diulang hingga tersisa satu kombinasi. Dengan cara ini, sumber daya komputasi dipusatkan secara bertahap pada kombinasi yang paling menjanjikan. Kombinasi terbaik yang tersisa pada akhir proses adalah konfigurasi yang dilaporkan untuk tiap dekoder di atas.
...
Potensi komplementaritas ini diuji melalui eksperimen *ensembling* yang menggabungkan dua sistem terbaik, yaitu arsitektur dua tahap (Conformer + 5-*gram* + LLaMA-2 7B) dengan WER 0,1556 dan arsitektur E2E Whisper-large-v3 dengan WER 0,1716. Untuk setiap ujaran, sebuah model regresi logistik memilih salah satu dari kedua keluaran berdasarkan skor keyakinan (*confidence*) masing-masing sistem. Proses pemilihan ini ditunjukkan pada Gambar IV.2.

![Proses ensembling dengan regresi logistik dua fitur](figures/fig_ensemble.png)

**Gambar IV.2** Proses *ensembling*. Skor keyakinan kedua sistem menjadi masukan model regresi logistik yang menghasilkan peluang p. Hipotesis E2E dipilih bila p melebihi 0,5, dan hipotesis dua tahap dipilih bila sebaliknya.

Masukan model adalah dua skor keyakinan, yaitu satu skor dari setiap sistem. Skor keyakinan arsitektur dua tahap diambil dari skor akhir daftar *n-best* yang dinormalkan terhadap panjang sinyal. Skor keyakinan arsitektur E2E diambil dari rata-rata log-probabilitas token yang dibangkitkan dekoder. Kedua skor distandarkan terlebih dahulu agar berada pada skala yang sebanding.

Kedua skor tersebut diproses oleh sebuah model regresi logistik menjadi sebuah probabilitas p, yaitu peluang bahwa keluaran E2E lebih baik daripada keluaran dua tahap. Apabila p melebihi 0,5, hipotesis E2E dipilih sebagai teks akhir. Apabila tidak, hipotesis dua tahap yang dipilih. Dengan demikian, keluaran akhir selalu berupa salah satu dari kedua hipotesis secara utuh, bukan gabungan kata dari keduanya.

Model regresi logistik dilatih dengan label biner pada setiap ujaran, yaitu bernilai satu apabila keluaran E2E memiliki kesalahan kata lebih sedikit dan bernilai nol apabila keluaran dua tahap lebih sedikit. Ujaran yang kedua sistemnya menghasilkan jumlah kesalahan sama dikeluarkan dari data pelatihan. Pelatihan dan pengujian dilakukan dengan validasi silang lima lipat langsung pada data uji. Cara ini dipilih karena model akustik cenderung terlalu cocok terhadap data latihnya sendiri sehingga skor keyakinan pada data latih tidak mencerminkan perilaku pada data uji. Dengan validasi silang, setiap ujaran selalu dinilai oleh model yang belum pernah melihatnya saat pelatihan sehingga tidak terjadi kebocoran data.

Hasilnya, model regresi logistik ini menurunkan WER menjadi 0,1488. Angka ini lebih baik daripada kedua sistem penyusunnya dan menjadi WER terendah pada tugas akhir ini, sekitar 0,7 poin persen di bawah sistem tunggal terbaik. Temuan ini menegaskan kedua arsitektur memang dapat saling melengkapi untuk meningkatkan akurasi. WER tersebut belum mencapai batas *oracle best-of-two* sebesar 0,1089, yaitu WER ideal seandainya untuk setiap ujaran selalu terpilih sistem dengan keluaran lebih baik. Selisih ini muncul karena keputusan model belum sempurna, yaitu pada sebagian ujaran terpilih sistem yang ternyata justru lebih buruk. Dengan demikian, ruang perbaikan yang tersisa terletak pada ketepatan pemilihan, bukan pada kemampuan kedua sistem itu sendiri.

Untuk mengilustrasikan pola komplementaritas tersebut secara konkret, Tabel IV.7 menyajikan beberapa contoh keluaran kedua sistem yang digabungkan, yaitu arsitektur dua tahap (Conformer + 5-*gram* + LLaMA-2 7B) dan E2E Whisper-large-v3, beserta transkrip referensinya.

**Tabel IV.7** Contoh keluaran arsitektur dua tahap dan E2E pada beberapa ujaran uji.

| Referensi                                    | Dua tahap                                    | E2E (Whisper-large-v3)                        | Keterangan                            |
| -------------------------------------------- | -------------------------------------------- | --------------------------------------------- | ------------------------------------- |
| i have seen it                               | i have done it                               | i have seen it                                | E2E benar, dua tahap salah            |
| do you have your bag                         | do you have your back                        | do you have your bag                          | E2E benar, dua tahap salah            |
| i believe it costs about ten dollars         | i believe it cost about ten dollars          | i believe it cost about ten dollars           | keduanya salah pada kata yang sama    |
| the measures already passed a senate committee | the measure already passed a senate committee | the measure already passed a second committee | keduanya salah dengan kesalahan berbeda |

Dua contoh pertama memperlihatkan kasus saat E2E memulihkan kesalahan yang dilakukan arsitektur dua tahap. Kesalahan tersebut umumnya berupa substitusi satu kata yang berbunyi mirip, seperti "bag" menjadi "back". Dua contoh terakhir memperlihatkan ujaran sulit yang gagal didekode kedua arsitektur. Pada kasus seperti ini, kesalahan kerap berupa perbedaan morfologi seperti bentuk jamak dan tunggal ("measures" menjadi "measure") atau kata yang fonemnya nyaris identik. Pola ini menegaskan sebagian kesalahan bersifat komplementer sehingga dapat diperbaiki dengan menggabungkan kedua arsitektur, sedangkan sebagian lainnya sulit bagi kedua arsitektur sekaligus.
