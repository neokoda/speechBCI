...
def rbox(cx, cy, label, ff, ee, italic_word=None):
    ...
    ax.text(cx, cy, label, ha="center", va="center", fontsize=12.5,
            color=TXT, wrap=True)
...
def pbox(cx, cy, label, ff, ee):          # parallelogram (I/O)
    ...
    ax.text(cx, cy, label, ha="center", va="center", fontsize=12.5, color=TXT)
...
rbox(LCX, ly[1], "BiRNN", YEL_F, YEL_E)
rbox(LCX, ly[2], "Probabilitas\nFonem", BLUE_F, BLUE_E)
rbox(LCX, ly[3], "Model Bahasa", YEL_F, YEL_E)
...
ax.text(bx+6.5, (top_y+bot_y)/2, "error\npropagation", ha="left", va="center",
        fontsize=11, style="italic", color=EP_RED, rotation=0)
